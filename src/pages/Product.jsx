...
const Product = () => {
  ...
  useEffect(() => {
    const calculateTotalPrice = () => {
      let basePrice = productData?.price || 0; // Ensure base price is defined
      let additionalCharges = 0; // Charges from containers and proteins

      // console.log('Base Price:', basePrice);

      // Check if a container is selected
      if (size) {
        const selectedSize = productData.containerSizes.find(
          (s) => s.size === size
        );
        if (selectedSize) {
          // Use container price instead of base price
          basePrice = selectedSize.price;
          // console.log('Selected Container Price:', selectedSize.price);
        }
      }
      ...
      if (selectedProtein.length > 0) {
        ...
        additionalCharges += proteinsTotal;
        // console.log('Selected Proteins Total Price:', proteinsTotal);
      }

      // If a container is selected, exclude the original base price
                  ...
                  {productData.spiceLevels.map((level) => (
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundColor:
                            selectedSpiceLevel === level ? 'red' : '#e2e8f0',
              ...
              <button
                onClick={() =>
                  addToCart(
                    ...
                    {
                      size,
                      price: productData.containerSizes.find(
                        (item) => item.size === size
                      )?.price,
                    },
                    selectedProtein
